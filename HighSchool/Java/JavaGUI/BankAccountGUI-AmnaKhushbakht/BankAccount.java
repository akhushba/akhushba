...
public class BankAccount implements java.io.Serializable{//, Serializable{ 
  ...
  public ArrayList getTransactions(LocalDateTime startTime, LocalDateTime endTime) {
    ...
    for(int i = 0; i < allTransactions.size(); i++){
      ...
      //defining a start time and an end time
        ...
        if (allTransactions.get(i).getTransactionTime().compareTo(endTime) <= 0 && allTransactions.get(i).getTransactionTime().compareTo(startTime) >= 0){
          selectTransactions.add(allTransactions.get(i));
        }
      }
    }
